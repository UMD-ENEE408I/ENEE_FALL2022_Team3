...
void read_adc (int& left, int& right, int& center){
  int adc_buf[13];
  int adc_left = 0;
  int adc_right = 0;
  int adc_center = 0;

  adc_buf[12] = adc1.readADC(0);  
  adc_buf[11] = adc2.readADC(0);
  adc_buf[10] = adc1.readADC(1);
  adc_buf[9]  = adc2.readADC(1);
  adc_buf[8]  = adc1.readADC(2);
  adc_buf[7]  = adc2.readADC(2);
  ...
  adc_right = adc_right/6; //<700
  adc_left = adc_left/6;  //<700
  adc_center = adc_buf[6]; //<650
  left = adc_left;
  right = adc_right;
  center = adc_center;
}
...
void loop() {
  ...
  while (true) {
    ...
    float target_v = 0.1; // forward velocity
    ...
    read_adc(left, right, center);

    if (center < 650) {
      target_theta = theta;
    } else if (left < 700){
      target_theta = theta + 0.5;
    } else if (right <700){
      target_theta = theta - 0.5;
    } else{
      target_theta = theta;
  ...
  Serial.print("Target_theta "); Serial.print(target_theta); Serial.print("\t");
  Serial.println();
