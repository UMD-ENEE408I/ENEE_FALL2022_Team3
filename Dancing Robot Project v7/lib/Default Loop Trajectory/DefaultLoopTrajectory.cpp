...
, float &BiOm) {
  ...
  static float target_theta = 0.0;
  static float kp_left = 200.0;
  static float ki_left = 20.0;
  static float kd_left = 20.0;
  static float kf_left = 10.0;
  ...
  if (check == 0) {
  // Loop period
    //target_period_ms; // Loop takes about 3 ms so a delay of 2 gives 200 Hz or 5ms

  // States used to calculate target velocity and heading
    //leminscate_a; // Radius, might wanna chance to 0.5
    //leminscate_t_scale; // speedup factor, 7/2pi
  ...
  // currently set to 360 deg/s compensation for 90 degrees of error
    ...
    float ayff = leminscate_t_scale;  //the time coefficient factored out in deriving
  switch(mode) {  //list of dance moves, not counting the hidden dance move
   case 0  :
   ...
   case 6  :
    ...
    float ay = update_pid(dt, 2.0, 2.0, 2.0, y, posy, integral_error_pos_y, max_integral_error_pos_y, last_y); //need to be updated
    ax = axff;
    ay = ayff;
