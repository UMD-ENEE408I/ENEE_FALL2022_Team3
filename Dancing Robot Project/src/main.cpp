...
void loop() {
  ...
  //default circle is 0
  if (brain.move_req != last_mode)  {
    start = 0;
  }
  float* speedangle;//2 variable pointer[0] is v, [1] is w
  speedangle = defaultLoop(enc1, enc2, start, brain1.move_req);  //TrajectoryTracking loop, should later be modified to change loop shape
