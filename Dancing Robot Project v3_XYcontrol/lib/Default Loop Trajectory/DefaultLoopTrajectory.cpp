...
float* defaultLoop(Encoder& enc1, Encoder& enc2, int check, int mode, float posx, float posy, float newx, float newy) {
  ...
  switch(mode) {
   ...
   case 6  :
    ...
    float ax = update_pid(dt, 200.0, 20.0, 20.0, x, posx, integral_error_pos_x, max_integral_error_pos_x, last_x); //variables might 
    float ay = update_pid(dt, 200.0, 20.0, 20.0, y, posy, integral_error_pos_y, max_integral_error_pos_x, last_y); //need to be updated
